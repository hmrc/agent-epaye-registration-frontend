...
class YourContactNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  "YourContactName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val someContactDetails: YourContactDetails = YourContactDetails("cn", Some("email@address.com"), Some("01 23 45"))

      val userAnswers = UserAnswers(userAnswersId).set(YourContactDetailsPage, someContactDetails).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      val yourContactDetails = YourContactDetails("cn", Some("email@address.com"), Some("01 23 45"))
      ...
      running(application) {
        val request =
          FakeRequest(POST, yourContactDetailsRoute)
            .withFormUrlEncodedBody("contactName" -> yourContactDetails.contactName)
